...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # as the search start, if encountered a node that is one of the four corners, remove it
        # resulting a lenth of 0 of the second arug of the state
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                # remove the corners that's getting eaten
                cornerData = state[1]
                cornerData = tuple(x for x in cornerData if x != (nextx, nexty))

                #append newState with new xy and cornerData
